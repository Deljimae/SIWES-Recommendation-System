...
async function generateRecommendationsForUser(req, res) {
  ...
  try {
    ...
    if (
      !user.Profile ||
      !user.Profile[0].interests ||
      user.Profile[0].interests.length === 0
    ) {
      ...
      return successResponse(
        res,
        "Please update your interests in the profile section",
        { recommendations: [] }
      );
    }
    ...
    if (
      user.Profile ||
      user.Profile[0].career_goals ||
      user.Profile[0].career_goals.length !== 0
    ) {
    ...
    let userEmbedding;
    try {
      const embeddingsData = await getEmbeddingsFromPythonService(
        combinedGoals
      );
      userEmbedding = embeddingsData.embeddings;
    } catch (embeddingError) {
      console.error(
        `Error getting user embedding for user ${user.uuid}:`,
        embeddingError
      );
    ...
    try {
      companyEmbeddingsData = await getEmbeddingsFromPythonService(
        companyIndustries
      );
    } catch (embeddingError) {
    ...
    for (let i = 0; i < companies.length; i++) {
      company_embeddings[companies[i].uuid] =
        companyEmbeddingsData.embeddings[i];
    }
    ...
    return successResponse(res, "Recommendations generated successfully.", {
      recommendations,
    });
  } catch (error) {
    console.error("Error generating recommendations:", error);
    return customResponse(res, 500, false, "An unexpected error occurred.", {
      error: error.message,
    });
  }
}
